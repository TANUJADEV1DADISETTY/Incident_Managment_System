...
RESPONSE=$(curl -s -X POST http://region-us:8080/incidents \
...
EU_INCIDENT=$(curl -s http://region-eu:8081/incidents/$INCIDENT_ID)
APAC_INCIDENT=$(curl -s http://region-apac:8082/incidents/$INCIDENT_ID)
...
curl -s -X PUT http://region-us:8080/config/urls \
  -H "Content-Type: application/json" \
  -d '{"urls": ["http://region-apac:8082"]}' > /dev/null

curl -s -X PUT http://region-eu:8081/config/urls \
  -H "Content-Type: application/json" \
  -d '{"urls": ["http://region-apac:8082"]}' > /dev/null
...
US_UPDATE_RES=$(curl -s -X PUT http://region-us:8080/incidents/$INCIDENT_ID \
  -H "Content-Type: application/json" \
...
EU_UPDATE_RES=$(curl -s -X PUT http://region-eu:8081/incidents/$INCIDENT_ID \
  -H "Content-Type: application/json" \
...
FINAL_EU_INCIDENT=$(curl -s http://region-eu:8081/incidents/$INCIDENT_ID)
